...
def post_new():
    post_new_form = PostForm()
    if post_new_form.validate_on_submit():
        post = get_post_info(post_new_form)
        db.session.add(post)
        db.session.commit()
        flash('Post created.', 'success')
        return redirect(url_for('admin.post'))
    return render_template('admin/post/post-new.html', post_new_form=post_new_form)
